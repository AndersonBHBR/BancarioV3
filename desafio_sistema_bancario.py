...
class Saque(Transacao):
    def __init__(self, valor):
        self.valor = valor
        self.tipo = "Saque"
        # Usar o formato de data utilizado no Histórico
        self.data = datetime.utcnow().strftime("%d-%m-%Y %H:%M:%S")
...
def recuperar_conta_cliente(cliente):
    ...
    for idx, conta in enumerate(cliente.contas):
        print(f"{idx + 1}: {conta.numero}")

    escolha = int(input(f"Digite o número da conta (1-{len(cliente.contas)}): "))
